<template>
	<div class="admin-profile-page">
		...
		<div v-else-if="!profileMe" class="admin-profile-loading">
			無法載入個人資料，請重新登入。
		</div>
		<template v-else>
			...
			<div
				...
				class="admin-profile-tab-content"
			>
				...
				<div class="admin-profile-editor-wrap">
					<ClientOnly>
						<AdminMilkdownEditor
							...
							@ready="onEditorReady"
						/>
						<div v-else class="admin-profile-editor-loading">
							載入編輯器…
						</div>
					</ClientOnly>
				</div>
...
const { uploadImage: uploadBanner, uploading: bannerUploading } =
	useUploadImage();
const { uploadImage: uploadAvatar, uploading: avatarUploading } =
	useUploadImage();
...
function buildFrontmatter(): string {
	...
	if (form.social.github || form.social.twitter || form.social.website) {
		...
		lines.push(
			`  twitter: "${(form.social.twitter || "").replace(/"/g, '\\"')}"`,
		);
		if (form.social.website)
			lines.push(`  website: ${form.social.website}`);
	}
	lines.push("---");
	return lines.join("\n");
}
...
.admin-profile-banner.empty .admin-profile-banner-placeholder {
	...
	background: linear-gradient(
		135deg,
		color-mix(in srgb, var(--color-primary) 25%, var(--color-bg-tertiary))
			0%,
