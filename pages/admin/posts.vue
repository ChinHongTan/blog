<template>
	<div class="admin-list-page">
		...
		<template v-else>
			...
			>
				...
				<div class="admin-draft-cards">
					<div
						...
						:key="d.key"
						class="admin-draft-card admin-draft-card-local"
					>
						<NuxtLink
							:to="d.editorLink"
						...
						>
							<span
								class="admin-draft-card-badge admin-draft-card-badge-local"
								:class="{
							...
							<span class="admin-draft-card-title">{{
								d.title
							}}</span>
							<p class="admin-draft-card-excerpt">
								{{ d.excerpt }}
							</p>
							<span class="admin-draft-card-time">{{
								d.relativeTime
							}}</span>
						</NuxtLink>
						<button
							type="button"
							class="admin-draft-card-trash"
							aria-label="捨棄本機草稿"
							@click.prevent="openDiscardConfirm(d)"
						>
			...
			<!-- GitHub 草稿（僅顯示我的） -->
			<section
				v-if="githubDraftsMine.length > 0"
				class="admin-continue-writing"
			>
				<h3 class="admin-continue-writing-title">GitHub 草稿</h3>
				<div class="admin-draft-cards">
					<div
						v-for="g in githubDraftsMine"
						:key="g.path"
						class="admin-draft-card admin-draft-card-github"
					>
						<NuxtLink
							:to="g.editorLink"
							class="admin-draft-card-link"
						>
							<span
								class="admin-draft-card-badge admin-draft-card-badge-github"
								:class="{
									'admin-draft-card-badge-has-local':
										hasLocalDraftForPath(g.path),
								}"
							>
								{{
									hasLocalDraftForPath(g.path)
			...
			>
				<div
					v-if="discardConfirmDraft"
					class="admin-discard-confirm-object-card"
				>
					<Icon
						name="heroicons:document-text"
						size="18"
						class="admin-discard-confirm-object-icon"
						aria-hidden="true"
					/>
					<span class="admin-discard-confirm-object-title">{{
						discardConfirmDraft.title
					}}</span>
				</div>
				<template #actions>
					<button
						type="button"
						class="ui-btn ui-btn-ghost admin-discard-cancel"
						@click="discardConfirmDraft = null"
					>
						取消
					</button>
					<button
						type="button"
						class="ui-btn ui-btn-danger admin-discard-submit"
			...
			<div v-else class="admin-posts-table-wrap">
				<table class="admin-posts-table" role="table">
					<thead>
						<tr>
							<th
								scope="col"
								class="admin-posts-th admin-posts-th-author"
							>
								作者
							</th>
							<th
								scope="col"
								class="admin-posts-th admin-posts-th-title"
							>
								標題
							</th>
							<th
								scope="col"
								class="admin-posts-th admin-posts-th-status"
							>
								狀態
							</th>
							<th
								scope="col"
								class="admin-posts-th admin-posts-th-series"
							>
								系列
							</th>
							<th
								scope="col"
								class="admin-posts-th admin-posts-th-date"
							>
								日期
							</th>
							<th
								scope="col"
								class="admin-posts-th admin-posts-th-actions"
							>
								操作
							</th>
						</tr>
					...
					<tbody>
						<tr
							...
							:key="row.path || row.draftKey"
							class="admin-posts-tr"
						>
							<td class="admin-posts-td admin-posts-td-author">
								<NuxtLink
									:to="row.editorLink"
									class="admin-posts-row-link"
								>
									<span
										class="admin-posts-avatar-wrap"
										aria-hidden="true"
									>
										<img
										...
										<span
											...
											>{{ row.authorInitial }}</span
										>
									</span>
									<span class="admin-posts-author-name">{{
							...
							<td class="admin-posts-td admin-posts-td-status">
								<span
									class="admin-posts-badge"
									:class="row.statusClass"
									>{{ row.statusLabel }}</span
								>
							</td>
							<td class="admin-posts-td admin-posts-td-series">
								<div
									v-if="row.series.length > 0"
									class="admin-posts-series-list"
								>
									<span
										v-for="seriesName in row.series"
										:key="`${row.path || row.draftKey}-${seriesName}`"
										class="admin-posts-series-chip"
									>
										{{ seriesName }}
									</span>
								</div>
								<span v-else class="admin-posts-series-empty"
									>—</span
								>
							</td>
							...
							<td class="admin-posts-td admin-posts-td-actions">
								...
								>
									<Icon
										...
										size="18"
									/>
									<span class="admin-posts-action-label"
										>編輯</span
									>
								</NuxtLink>
								<NuxtLink
				...
				<p
					v-if="
						...
						githubDraftsMine.length === 0
					"
					class="admin-posts-empty"
				>
					尚無文章
				</p>
				<p
...
// Auth state
const { user, loading: authLoading, authError, login } = useAdminAuth();
const permissionMessage = computed(() => {
	if (
		typeof route.query.authError === "string" &&
		route.query.authError === "no_write"
	) {
...
function loadLocalDrafts() {
	...
	for (let i = 0; i < localStorage.length; i++) {
		...
		try {
			...
			const editorLink =
				pathPart === "new"
					? "/admin/editor?type=post"
					: `/admin/editor?type=post&path=${encodeURIComponent(pathPart)}`;
...
const githubDraftsMine = computed<GitHubDraftCardItem[]>(() => {
	...
	return index
		.filter(
			(p) => p.draft && p.author.toLowerCase() === authorId.toLowerCase(),
		)
		.map((p) => {
...
const tableRows = computed<TableRow[]>(() => {
	...
	for (const p of index) {
		...
		const statusLabel = hasLocal
			? "本機草稿"
			: p.draft
				? "GitHub 草稿"
				: "已發布";
		const statusClass = hasLocal
			? "admin-posts-badge-local"
			: p.draft
				? "admin-posts-badge-draft"
				: "admin-posts-badge-published";
		const isMine =
			!!authorId && p.author.toLowerCase() === authorId.toLowerCase();
		const dateMs = new Date(p.date).getTime();
		rows.push({
			path: p.path,
			editorLink: `/admin/editor?type=post&path=${encodeURIComponent(p.path)}`,
			...
			authorAvatar: p.authorAvatar || "",
			authorInitial:
				(p.authorDisplayName || p.author || "?")[0]?.toUpperCase() ??
				"?",
			statusLabel,
			statusClass,
...
async function load() {
	...
	try {
		const deletedPath =
			typeof route.query.deletedPath === "string"
				? route.query.deletedPath
				: "";
		const fetchList = (t: number) =>
			$fetch<{ posts: PostIndexItem[]; series: string[] }>(
				"/api/admin/posts-index",
				{ query: { t } },
			).catch(() => ({ posts: [], series: [] }));
		const result = await fetchList(Date.now());
		postIndex.value = result.posts;
		if (deletedPath && Array.isArray(postIndex.value)) {
			// GitHub content API can be briefly stale after delete/move; retry shortly once.
			const stillPresent = postIndex.value.some(
				(p) => p.path === deletedPath,
			);
			if (stillPresent) {
				await new Promise((resolve) => setTimeout(resolve, 700));
				const retryResult = await fetchList(Date.now());
				postIndex.value = retryResult.posts;
			}
		}
