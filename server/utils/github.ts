	...
	if (!secret) {
		if (process.env.NODE_ENV === "production") {
			throw createError({
				statusCode: 500,
				message:
					"NUXT_COOKIE_ENCRYPTION_SECRET must be configured in production",
			});
		}
...
export function getRepoOwnerRepo(event: H3Event): {
	owner: string;
	repo: string;
} {
	const config = useRuntimeConfig(event);
...
export function getRepoBranch(event: H3Event): string {
	const config = useRuntimeConfig(event);
	const branch = (config.public?.githubBranch ||
		config.githubBranch ||
		"main") as string;
...
export function validateAdminPath(path: string): void {
	...
	if (!isAllowed) {
		throw createError({
			statusCode: 403,
			message: `Access denied: path must start with ${ALLOWED_PATH_PREFIXES.join(" or ")}`,
		});
	}
}
...
export function isPostContentPath(path: string): boolean {
	const normalized = path.replace(/^\/+/, "");
	return (
		normalized.startsWith("content/blog/") ||
		normalized.startsWith("content/drafts/")
	);
}
...
export function extractFrontmatterObject(content: string): Record<string, any> {
	...
	for (const line of lines) {
		...
		if (arrayItemMatch && currentArrayKey) {
			if (!Array.isArray(obj[currentArrayKey])) {
				obj[currentArrayKey] = [];
			}
			obj[currentArrayKey].push(arrayItemMatch[1].trim());
			continue;
		}
...
export function isOnlyFrontmatterChange(
	newContent: string,
	oldContent: string,
	allowedProps: string[],
): boolean {
	const oldFm = extractFrontmatterObject(oldContent);
	const newFm = extractFrontmatterObject(newContent);

	// Extract body content (anything after the second ---)
	const bodyRegex = /^---\s*\n[\s\S]*?\n---\s*\n?([\s\S]*)$/;
	const oldBodyMatch = oldContent.match(bodyRegex);
	const newBodyMatch = newContent.match(bodyRegex);
	const oldBody = oldBodyMatch ? oldBodyMatch[1].trim() : oldContent.trim();
	const newBody = newBodyMatch ? newBodyMatch[1].trim() : newContent.trim();
	...
	// Check frontmatter: find all keys that changed
...
function extractAuthorGithubLogin(raw: string): string | null {
	...
	return githubUrlToLogin(
		(ghMatch[1] ?? "").trim().replace(/^["']|["']$/g, ""),
	);
}
			...
			const results = await Promise.all(
				mdFiles.map(async (f) => {
					try {
						...
						return {
							path: f.path,
							githubLogin: extractAuthorGithubLogin(raw),
						};
					} catch {
						return { path: f.path, githubLogin: null };
					}
...
export async function resolveCurrentAuthorId(
	event: H3Event,
): Promise<string | null> {
	...
	return withRequestCache<string | null>(
		...
		async () => {
			...
			try {
				const authorIndex = await getAuthorGithubIndex(
					octokit,
					owner,
					repo,
				);
