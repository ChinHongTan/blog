	...
	try {
		if (isPostContentPath(path)) {
			...
			if (!me) {
				throw createError({
					statusCode: 403,
					message: "Unable to resolve current author identity",
				});
			}
			const incomingAuthor =
				extractFrontmatterAuthor(content)?.toLowerCase();
			if (incomingAuthor && incomingAuthor !== me) {
				throw createError({
					statusCode: 403,
					message: "You can only save your own posts",
				});
			}
			...
			try {
				...
				if (
					...
					rawExistingContent = Buffer.from(
						existing.content,
						"base64",
					).toString("utf-8");
					existingAuthor =
			...
			if (!existingAuthor && !incomingAuthor) {
				throw createError({
					statusCode: 400,
					message: "Post frontmatter must include author",
				});
			}
		}
