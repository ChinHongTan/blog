...
const authorDirectory = computed<Record<string, AuthorCollectionItem>>(() => {
	...
	const records = (authorEntries.value ??
		[]) as unknown as AuthorCollectionItem[];
...
// Keep the active TOC link visible within the sidebar's own scroll container,
// without affecting the page scroll position.
const scrollActiveLinkIntoView = () => {
	if (!activeId.value) return;
	const linkEl = tocLinkRefs.value[activeId.value];
	if (!linkEl) return;
	const sidebar = linkEl.closest(".right-sidebar") as HTMLElement | null;
	if (!sidebar) return;
	...
	if (linkRect.top < sidebarRect.top + margin) {
		sidebar.scrollBy({
			top: linkRect.top - sidebarRect.top - margin,
			behavior: "smooth",
		});
	} else if (linkRect.bottom > sidebarRect.bottom - margin) {
		sidebar.scrollBy({
			top: linkRect.bottom - sidebarRect.bottom + margin,
			behavior: "smooth",
		});
	}
};
...
const setupScrollObserver = () => {
	...
	// Observe all h2 and h3 elements (excluding ones inside info-box callouts)
	const headings = Array.from(
		document.querySelectorAll(".post-content h2, .post-content h3"),
	).filter((heading) => !heading.closest(".info-box"));
...
const authors = computed(() => {
	...
	props.allPosts.forEach((post: unknown) => {
		...
		if (postData.author) {
			authorMap.set(
				postData.author,
				(authorMap.get(postData.author) || 0) + 1,
			);
		}
	});
...
<template>
	<aside :class="['right-sidebar', { 'post-sidebar': isPostPage }]">
		...
		<div
			v-if="isPostPage && toc.length > 0"
			class="sidebar-section toc-section"
		>
			...
			<nav class="toc-links">
				...
				<a
					...
					:ref="
						(element) =>
							setTocLinkRef(link.id, element as Element | null)
					"
					:href="`#${link.id}`"
					:class="[
						'toc-link',
		...
		<!-- Posts by Year -->
		<div
			v-if="!isPostPage && postsByYear.length > 0"
