...
解決這個問題的其中一個方法就是使用 **[`Lavalink`](https://github.com/lavalink-devs/Lavalink)**。它是一個專門為 Discord 機器人打造的獨立 Java 音訊伺服器。機器人只負責接收使用者的指令（例如 `/play`），Lavalink 則扛下所有髒活：解析 YouTube 網址、繞過限制、解碼音訊，並直接把串流送到 Discord 的語音頻道裡。兩者之間只透過 WebSocket 和 REST API 進行溝通。
