...
當資訊框的**第一行是一個標題**（例如 `### 補充説明：...`）時，該資訊框在網站上會**自動變成可折疊（collapsible）**。讀者預設只會看到標題，點擊標題才會展開完整內容。
...
也可以在空白行輸入 `/` 叫出**斜線選單**，直接用鍵盤搜尋並插入區塊（標題、列表、程式碼、圖片、表格、資訊框等），不用離開鍵盤。

## 編輯器快捷鍵

以下快捷鍵在所見即所得編輯器中有效。`Mod` 在 Windows/Linux 上是 `Ctrl`，在 Mac 上是 `Cmd`。
...
| 快捷鍵 | 功能 |
| --- | --- |
| `Enter` | 新增同層列表項（在空白項目按下會退出列表） |
| `Tab` 或 `Mod + ]` | 增加縮排（建立巢狀列表） |
| `Shift + Tab` 或 `Mod + [` | 減少縮排 / 退出列表項 |
| `Backspace`（在項目開頭）| 退出列表 |

**小提示**：若要把列表項中間的文字變成獨立段落（脫離列表），先按 `Enter` 分割，再按 `Shift + Tab` 即可。
